...
def lds_apply_qtwebengine_realtime_flags():
    try:
        ...
        try:
            print("[LiteDesktopStudio] apply QtWebEngine realtime flags failed:", repr(e))
        except Exception:
            pass


lds_apply_qtwebengine_realtime_flags()
...
try:
    ...
    for _lds_flag in ("--disk-cache-size=1", "--media-cache-size=1", "--disable-gpu-shader-disk-cache"):
        if _lds_flag not in _lds_flags:
            _lds_flags = (_lds_flags + " " + _lds_flag).strip()
    os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = _lds_flags
except Exception:
    pass
...
class LiteDeskStudio(QMainWindow):
    ...
    def build_center_panel(self):
        ...
        other_tools_label = QLabel(lds_tr("🧰 その他のツール"))
        other_tools_label.setObjectName("SectionTitle")
        layout.addWidget(other_tools_label)

        self.other_tools_text = QTextEdit()
        self.other_tools_text.setReadOnly(True)
        self.configure_readable_help_text(self.other_tools_text, min_height=150, fixed_height=170)
        self.other_tools_text.setObjectName("HelpBox")
        self.other_tools_text.setPlainText(lds_tr(
            "JSHTML package、画像変換、HTML/JSON/JavaScript編集、診断などの補助機能を開きます。\n\n"
            "右側のプロパティに詰め込みすぎず、今後ツールが増えてもこの画面から管理できるようにします。"
        ))
        self.performance_text = self.other_tools_text
        layout.addWidget(self.other_tools_text)

        self.btn_other_tools = QPushButton(lds_tr("🧰 その他のツールを開く"))
        self.btn_other_tools.setMinimumHeight(38)
        self.btn_other_tools.clicked.connect(self.open_other_tools_dialog)
        layout.addWidget(self.btn_other_tools)
    ...
    def build_property_panel(self):
        ...
        self.prop_visualizer_style = QComboBox()
                # Removed/legacy styles are intentionally not added to this combo box.
    ...
    def open_other_tools_dialog(self):
        try:
            dialog = OtherToolsDialog(self.canvas, self)
            dialog.exec()
        except Exception as e:
            QMessageBox.warning(self, lds_tr("その他のツール"), str(e))
...
class DesktopCanvas(QWidget):
    ...
    def _effective_effect_frame_interval_ms(self) -> int:
        ...
        try:
            ...
            fps_values = []
        fps = min(fps_values) if fps_values else 45
    ...
    def open_studio_for_widget(self, widget):
        if widget is None:
            return False
        try:
            ...
            try: print("[LiteDesktopStudio] open_studio_for_widget failed:", repr(e))
            except: pass
            return False

    def open_studio_for_point(self, pos):
        try:
            ...
            if widget is None:
                ...
                self.last_right_click_widget = None
                return False
            self.last_right_click_widget = widget
            return self.open_studio_for_widget(widget)
        except Exception as e:
            try: print("[LiteDesktopStudio] open_studio_for_point failed:", repr(e))
            except: pass
            return False
